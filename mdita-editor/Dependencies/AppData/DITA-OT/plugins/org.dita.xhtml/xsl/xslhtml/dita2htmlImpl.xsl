...
<xsl:variable name="ttype">
      <xsl:choose>
          <xsl:when test="contains(//@class, 'learningContent/learningContent')">LC</xsl:when>
          <xsl:when test="contains(//@class, 'learningOverview/learningOverview')">LO</xsl:when>
          <xsl:when test="contains(//@class, 'learningSummary/learningSummary')">LS</xsl:when>
          <xsl:when test="contains(//@class, 'learningAssessment/learningAssessment')">LA</xsl:when>
          <xsl:otherwise>AAAAAAAAAAA<xsl:value-of select="@class" /></xsl:otherwise>
      </xsl:choose>
</xsl:variable>
...
<div class="container">
 ...
 <xsl:value-of select="$newline"/><xsl:comment>TopicType ===== <xsl:value-of select="$ttype"/> === END ===</xsl:comment><xsl:value-of select="$newline"/>
<xsl:value-of select="$newline"/>

<!-- IRVAS -->
<xsl:comment>init slides</xsl:comment>
<xsl:call-template name="addSliderInit"/>
...
-->
  <xsl:variable name="chapNumber">
      <xsl:if test="not($ttype='LO')">
         <xsl:value-of select="substring-after(../@id,'-')"/>
<!--  - <xsl:apply-templates/> -->
      </xsl:if>
  </xsl:variable>
  <xsl:element name="div">
      <xsl:attribute name="class">slider_header_<xsl:value-of select="$chapNumber"/></xsl:attribute>
      <xsl:value-of select="substring-after(../@id,'-')"/> - <xsl:apply-templates/>
  </xsl:element>
  <xsl:value-of select="$newline"/>
</xsl:template>
...
<!-- IRVAS -->
  <xsl:call-template name="addSliderNavigation"/> 
</div><xsl:value-of select="$newline"/>

</xsl:if>
<!-- ====================================LO topics=========================================================== -->
<xsl:if test="$ttype='LO'">
<div id="slides">
  <xsl:call-template name="commonattributes"/>
  <xsl:call-template name="setidaname"/>
  <xsl:apply-templates select="*[contains(@class, ' ditaot-d/ditaval-startprop ')]" mode="out-of-line"/>
...
<!-- IRVAS
  <xsl:apply-templates select="following-sibling::*[contains(@class, ' topic/related-links ')]" mode="prereqs"/>
-->
  <div class="section wrapper_home">
      <div class="lekcija">Lekcija <xsl:value-of select="substring-after(../@id,'-')"/></div>
      <div class="hometitle"><xsl:value-of select="../title"/></div>
      <div class="homesubtitle"><xsl:value-of select="../shortdesc/draft-comment[@disposition='Author']"/><xsl:value-of select="$newline"/></div>
  </div>
  <div class="section wrapper1">
     <div class="sectiontitle">SADRAJ</div> 
     <div class="section5columns"><xsl:value-of select="$newline"/>
         <xsl:for-each select="*[contains(@class, ' topic/section')]">
          <xsl:if test="position() &lt; 6">
           <xsl:variable name="secid" select="lower-case(substring-after(./@id,'-'))"/>
           <div class="content"><xsl:value-of select="$newline"/>
             <div class="sadrzaj_column"><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
                     <xsl:attribute name="class">sadrzaj_subtitle_<xsl:value-of select="$secid"/></xsl:attribute>
                     <xsl:value-of select="substring-after(./@id,'-')"/><xsl:value-of select="$newline"/>
                  </xsl:element><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
                     <xsl:attribute name="class">sadrzaj_chaptertitle_<xsl:value-of select="$secid"/></xsl:attribute>
                     <xsl:value-of select="./title"/><xsl:value-of select="$newline"/>
                  </xsl:element><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
                     <xsl:attribute name="class">sadrzaj_<xsl:value-of select="$secid"/></xsl:attribute>
                      <xsl:if test= "not(contains(./@id, 'S-UVOD'))">
                          <xsl:apply-templates mode="all-but-title" />
                      </xsl:if><xsl:value-of select="$newline"/>
                  </xsl:element><xsl:value-of select="$newline"/>
             </div><xsl:value-of select="$newline"/>
         </div>
         </xsl:if>
  ...
  <xsl:variable name="secCountTOC" select="count(section[not(contains(normalize-space(./@id),'S-UVOD'))])"/>
  <xsl:variable name="secCount2" select="count(section)-count(section[ contains(normalize-space(./@id),'S-UVOD')])+1"/>

  <xsl:if test="$secCountTOC &gt; 4"> 
  <div class="section wrapper1">
     <div class="sectiontitle">...</div> 
     <div class="section5columns"><xsl:value-of select="$newline"/>
           <xsl:for-each select="*[contains(@class, ' topic/section')]">
          <xsl:if test="(position() &gt; 5) and (position() &lt; 11) and ( not(contains(./@id,'S-UVOD')))">
           <xsl:variable name="secid" select="lower-case(substring-after(./@id,'-'))"/>
           <div class="content"><xsl:value-of select="$newline"/>
             <div class="sadrzaj_column"><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
...
<xsl:for-each select="(//*)[position()&lt;=$maxCount]">
 <xsl:if test="(position() &gt;= $secCount2) and ( not(contains(./@id,'S-UVOD')))">
           <xsl:variable name="secid" select="position()"/>
           <div class="content"><xsl:value-of select="$newline"/>
             <div class="sadrzaj_column"><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
                     <xsl:attribute name="class">sadrzaj_subtitle_0<xsl:value-of select="$secid"/></xsl:attribute>
                     0<xsl:value-of select="$secid"/><xsl:value-of select="$newline"/>
                  <xsl:value-of select="$newline"/>
                  </xsl:element><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
                     <xsl:attribute name="class">sadrzaj_chaptertitle_0<xsl:value-of select="$secid"/></xsl:attribute>
                     <xsl:value-of select="./title"/><xsl:value-of select="$newline"/>
                  </xsl:element><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
                     <xsl:attribute name="class">sadrzaj_0<xsl:value-of select="$secid"/></xsl:attribute>
                      <xsl:if test= "not(contains(./@id, 'S-UVOD'))">
                       <!--   <xsl:apply-templates mode="all-but-title" /> -->
                      </xsl:if><xsl:value-of select="$newline"/>
                  </xsl:element><xsl:value-of select="$newline"/>
  ...
  <xsl:if test="$secCountTOC &gt; 9">   
  <div class="section wrapper1">
     <div class="sectiontitle">...</div>
     <div class="section5columns"><xsl:value-of select="$newline"/>
           <xsl:for-each select="*[contains(@class, ' topic/section')]">
          <xsl:if test="(position() &gt; 10) and (position() &lt; 16) and ( not(contains(./@id,'S-UVOD')))">
           <xsl:variable name="secid" select="lower-case(substring-after(./@id,'-'))"/>
           <div class="content"><xsl:value-of select="$newline"/>
             <div class="sadrzaj_column"><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
...
<xsl:variable name="maxCount" select="'14'"/>
<xsl:for-each select="(//*)[position()&lt;=$maxCount]">
 <xsl:if test="(position() &gt;= $secCount2) and ( not(contains(./@id,'S-UVOD')))">
           <xsl:variable name="secid" select="position()"/>
           <div class="content"><xsl:value-of select="$newline"/>
             <div class="sadrzaj_column"><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
                     <xsl:attribute name="class">sadrzaj_subtitle_<xsl:value-of select="$secid"/></xsl:attribute>
                     <xsl:value-of select="$secid"/><xsl:value-of select="$newline"/>
                  <xsl:value-of select="$newline"/>
                  </xsl:element><xsl:value-of select="$newline"/>
                  <xsl:element name="div">
                     <xsl:attribute name="class">sadrzaj_chaptertitle_<xsl:value-of select="$secid"/></xsl:attribute>
...
<xsl:if test="$ttype='LS'">
<div id="slides">
  <xsl:call-template name="commonattributes"/>
  <xsl:call-template name="setidaname"/>
  <xsl:apply-templates select="*[contains(@class, ' ditaot-d/ditaval-startprop ')]" mode="out-of-line"/>
  <xsl:apply-templates select="preceding-sibling::*[contains(@class, ' topic/abstract ')]" mode="outofline"/>
  <div class="section wrapper_section_10">
     <div class="lekcija"><xsl:value-of select="../title"/><xsl:value-of select="$newline"/></div>
     <div class="hometitle"><xsl:value-of select="$newline"/></div>
     <div class="homesubtitle"><xsl:value-of select="$newline"/></div>
  </div>
  <xsl:apply-templates/>
  <xsl:apply-templates select="*[contains(@class, ' ditaot-d/ditaval-endprop ')]" mode="out-of-line"/>
<!-- IRVAS -->
...
<xsl:template mode="all-but-title" match="title"/>
<xsl:template mode="all-but-title" match="ul">
 <ul>
  <xsl:apply-templates/>
 </ul>
</xsl:template>
...
<xsl:template match="*[contains(@class, ' topic/shortdesc ')]" mode="outofline.abstract">
  <xsl:choose>
    <xsl:when test="preceding-sibling::*[contains(@class, ' topic/p ') or contains(@class, ' topic/dl ') or
										 ...
										 contains(@class, ' topic/br ') or
										 contains(@class, ' topic/video ') or
										 contains(@class, ' topic/audio ') or
										 contains(@class, ' topic/galleryimage ') or
										 contains(@class, ' topic/youtube ') or
...
<xsl:template match="*[contains(@class, ' topic/section ')]" name="topic.section">
  <xsl:if test="$ttype='LC'">
    <xsl:value-of select="$newline"/><xsl:comment>IRVAS LC SECTION</xsl:comment><xsl:value-of select="$newline"/>
    <div class="section">
      <xsl:call-template name="commonattributes"/>
      <xsl:call-template name="gen-toc-id"/>
      <xsl:call-template name="setidaname"/>
      ...
      <xsl:if test="contains(name(.), 'lcObjectives')">
          <xsl:element name="div">
             <xsl:attribute name="class">maintitle_chapter </xsl:attribute>
             <xsl:value-of select="substring-after(../../@id,'-')"/><xsl:value-of select="$newline"/> 
          </xsl:element>
     </xsl:if>
     </div><xsl:value-of select="$newline"/>
  ...
  <xsl:if test="$ttype='LO'">
    <xsl:if test= "contains(./@id, 'S-UVOD')">
    <xsl:value-of select="$newline"/><xsl:comment>IRVAS LO SECTION</xsl:comment><xsl:value-of select="$newline"/>
    <div class="sadrzaj_column">
      <xsl:call-template name="commonattributes"/>
      <xsl:call-template name="gen-toc-id"/>
      <xsl:call-template name="setidaname"/>
      <xsl:apply-templates select="."  mode="section-fmt" />
...
<xsl:template match="*[contains(@class, ' topic/p ')]" name="topic.p">
 ...
 <xsl:choose>
  <xsl:when test="descendant::*[contains(@class, ' topic/pre ')] or
       ...
       descendant::*[contains(@class, ' topic/br ')] or
...
<xsl:template match="*[contains(@class, ' topic/slides ')]" name="topic.slides">
 <!-- To ensure XHTML validity, need to determine whether the DITA kids are block elements.
      If so, use div_class="p" instead of p -->
 <xsl:choose>
  <xsl:when test="descendant::*[contains(@class, ' topic/pre ')] or
...
<xsl:template match="*[contains(@class,' topic/video ')]">
   	<video width="{@width}" height="{@height}" controls="true">
        <xsl:call-template name="commonattributes"/>
        <xsl:call-template name="setidaname"/>
...
<xsl:template match="*[contains(@class,' topic/galleryimage ')]">
<xsl:choose>
  <xsl:when test="descendant::*[contains(@class, ' topic/pre ')] or
       descendant::*[contains(@class, ' topic/ul ')] or
       descendant::*[contains(@class, ' topic/sl ')] or
       ...
       descendant::*[contains(@class, ' topic/simpletable ')]">
		<li><img src="{@src}"/>
		<p class="flex-caption"><b><xsl:value-of select="@title" /></b> <br/><xsl:value-of select="@description" /></p>
		</li>
     </xsl:when>
  <xsl:otherwise>
		<li><img src="{@src}"/>
...
<xsl:template match="*[contains(@class, ' topic/font ')]" name="topic.font">
 <!-- To ensure XHTML validity, need to determine whether the DITA kids are block elements.
      If so, use div_class="p" instead of p -->
 <xsl:choose>
  <xsl:when test="descendant::*[contains(@class, ' topic/pre ')] or
       ...
       descendant::*[contains(@class, ' topic/note ')] or
       descendant::*[contains(@class, ' topic/br ')] or
       descendant::*[contains(@class, ' topic/font ')] or
       descendant::*[contains(@class, ' topic/lines ')] or
       descendant::*[contains(@class, ' topic/fig ')] or
       descendant::*[contains(@class, ' topic/table ')] or
       descendant::*[contains(@class, ' topic/simpletable ')]">
...
<xsl:template match="*[contains(@class,' topic/youtube ')]">
    <iframe src="{@src}" frameborder="0" allowfullscreen="allowfullscreen">
...
<xsl:template match="*[contains(@class, ' topic/br ')]" name="topic.br">
 <!-- To ensure XHTML validity, need to determine whether the DITA kids are block elements.
      If so, use div_class="p" instead of p -->
 <xsl:choose>
  <xsl:when test="descendant::*[contains(@class, ' topic/pre ')] or
...
<!-- IRVAS ovo je ako se zeli link i u popup-u 
  <a class='inline' href="#inline_source">Pogledaj</a>
  <div style='display:none'>
    <div id='inline_source' style='padding:10px; background:#fff;'>
      <pre>
...
-->
      ...
      <xsl:variable name="noflines">
            <xsl:variable name="inputclass" select="tokenize(@outputclass, ' ')[contains(.,'noflines')]"/> 
            <xsl:value-of select="replace($inputclass, '.*[^.\d](\d*.?\d+)$', '$1')"/>
      </xsl:variable>
      <xsl:variable name="scheight">
         <xsl:value-of select="number($noflines+1) * 16"/>px
      </xsl:variable>
      <pre>
        <xsl:attribute name="class"><xsl:value-of select="name()"/></xsl:attribute>
        <xsl:call-template name="commonattributes"/>
        <xsl:call-template name="setscale"/>
        <xsl:call-template name="setidaname"/>
        <xsl:attribute name="style">max-height: <xsl:value-of select="$scheight"/></xsl:attribute>
        <xsl:apply-templates/>
      </pre>
  <xsl:apply-templates select="*[contains(@class, ' ditaot-d/ditaval-endprop ')]" mode="out-of-line"/>
...
<xsl:template match="*[contains(@class, ' topic/fig ')]" mode="fig-fmt">
  ...
  <div>
    ...
    <xsl:call-template name="setscale"/>
    <xsl:call-template name="setidaname"/>
<!-- POZICIJA desc/title pre slike IRVAS -->
...
<!-- POZICIJA desc/title posle slike IRVAS -->
    <xsl:call-template name="place-fig-lbl"/>
  </div>
  <xsl:apply-templates select="*[contains(@class, ' ditaot-d/ditaval-endprop ')]" mode="out-of-line"/>
  <xsl:value-of select="$newline"/>
...
<xsl:template match="*[contains(@class, ' topic/table ')]" mode="table-fmt">
  ...
  <xsl:choose>
    <xsl:when test="@frame = 'all' and $colsep = '0' and $rowsep = '0'">
      <table cellpadding="4" cellspacing="0" border="0" class="tableborder">
        <tr>
          <td>
            <xsl:value-of select="$newline"/>
...
<!-- IRVAS -->
      ...
      <xsl:if test="contains($ancestry,'section') and not(contains($ancestry,'sectiondiv'))">
         <xsl:choose>
         <xsl:when test="contains($ancestry,'lcObjectives')"> 
               <xsl:text> wrapper_section_</xsl:text><xsl:value-of select="$chapNumber"/>
         </xsl:when>
         <xsl:otherwise>
               <xsl:text> wrapper1</xsl:text>
         </xsl:otherwise>
       </xsl:choose>
      </xsl:if>
<!-- IRVAS -->
...
<xsl:template match="*[contains(@class, ' topic/section ')]/*[contains(@class, ' topic/title ')] | 
  ...
  <xsl:variable name="isLC">
       <xsl:if test="contains(name(parent::*), 'lcObjectives')">
          <xsl:value-of select="'maintitle'"/>
       </xsl:if>
  </xsl:variable>
  <!-- <xsl:element name="{$headLevel}">   IRVAS replace h2 to div -->
  <xsl:element name="div">
    <xsl:attribute name="class">sectiontitle </xsl:attribute>
    <xsl:call-template name="commonattributes">
      <xsl:with-param name="default-output-class" select="concat('sectiontitle ', $isLC)"/>
    </xsl:call-template>
    <xsl:apply-templates/>
  </xsl:element>
</xsl:template>
...
<xsl:param name="stringName"/>
  ...
  <xsl:choose>
    <!-- title -or- title & desc -->
    <xsl:when test="*[contains(@class, ' topic/title ')]">
       <br/><span class="figcap">
       <xsl:choose>      <!-- Hungarian: "1. Figure " -->
        <xsl:when test="( (string-length($ancestorlang) = 5 and contains($ancestorlang, 'hu-hu')) or (string-length($ancestorlang) = 2 and contains($ancestorlang, 'hu')) )">
         <!-- IRVAS <xsl:value-of select="$fig-count-actual"/><xsl:text>. </xsl:text> -->
         <xsl:call-template name="getString">
...
<!-- IRVAS -->
      <xsl:call-template name="addSliderSupp"/>
  ...
  <xsl:template match="*" mode="chapterBody">
    <body style="font-size: 13px; font-family: 'Open Sans',sans-serif; background-image: url(&quot;./css/bg-light.png&quot;); color: rgb(0, 0, 0);">
...
-->
  ...
  <xsl:template match="*[contains(@class, 'lcObjectivesGroup')]">
    <div class="maintitle_separator"><xsl:value-of select="$newline"/></div>
    <div class="mainsubtitle">
       <ul class="ul lcObjectivesGroup"> 
       <xsl:apply-templates/>
    </ul>
    </div>
  </xsl:template>
...
<xsl:template name="addSliderSupp">
    <xsl:variable name="empty" select="' '"/>
    <xsl:comment>part for slides</xsl:comment><xsl:value-of select="$newline"/>
    ...
    <xsl:element name="link">
       <xsl:attribute name="href">css/stylesheet.css</xsl:attribute>
       <xsl:attribute name="rel">stylesheet</xsl:attribute>
       <xsl:attribute name="type">text/css</xsl:attribute>
    </xsl:element> <xsl:value-of select="$newline"/>
    <xsl:element name="link">
       <xsl:attribute name="href">css/example.css</xsl:attribute>
       <xsl:attribute name="rel">stylesheet</xsl:attribute>
       <xsl:attribute name="type">text/css</xsl:attribute>
    </xsl:element> <xsl:value-of select="$newline"/>
    <xsl:element name="link">
       <xsl:attribute name="href">css/bmuslider.css</xsl:attribute>
       <xsl:attribute name="rel">stylesheet</xsl:attribute>
       <xsl:attribute name="type">text/css</xsl:attribute>
    </xsl:element> <xsl:value-of select="$newline"/>
    <xsl:element name="link">
       <xsl:attribute name="href">css/smoothzoom.css</xsl:attribute>
       <xsl:attribute name="rel">stylesheet</xsl:attribute>
       <xsl:attribute name="type">text/css</xsl:attribute>
    </xsl:element> <xsl:value-of select="$newline"/>
    <xsl:element name="link">
       <xsl:attribute name="href">css/colorbox.css</xsl:attribute>
       <xsl:attribute name="rel">stylesheet</xsl:attribute>
       <xsl:attribute name="type">text/css</xsl:attribute>
    </xsl:element> <xsl:value-of select="$newline"/>
  <xsl:element name="link">
       <xsl:attribute name="href">jscripts/flexslider.css</xsl:attribute>
       <xsl:attribute name="rel">stylesheet</xsl:attribute>
       <xsl:attribute name="type">text/css</xsl:attribute>
    </xsl:element> <xsl:value-of select="$newline"/>
    <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">http://code.jquery.com/jquery-1.9.1.min.js</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element><xsl:value-of select="$newline"/>
    <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">jscripts/jquery.slides.min.js</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element><xsl:value-of select="$newline"/>
    <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">jscripts/easing.js</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element><xsl:value-of select="$newline"/>
    <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">jscripts/smoothzoom.min.js</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element><xsl:value-of select="$newline"/>
    <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">jscripts/jquery.colorbox.js</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element><xsl:value-of select="$newline"/>
    <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">https://cdn.rawgit.com/google/code-prettify/master/loader/run_prettify.js</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element><xsl:value-of select="$newline"/>
	 <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">jscripts/jquery.flexslider.js</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element><xsl:value-of select="$newline"/>
    <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">http://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS-MML_SVG&amp;delayStartupUntil=configured</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element>
<xsl:value-of select="$newline"/>
    <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">http://cdn.mathjax.org/mathjax/latest/MathJax.js</xsl:attribute>
      <xsl:value-of select="$empty"/>
    </xsl:element><xsl:value-of select="$newline"/>
 <xsl:element name="script">
  MathJax.Hub.Config({
 ...
 SVG: { linebreaks: { automatic: true, width: "100% container" },scale: 90,preferredFonts: "TeX"
...
<xsl:value-of select="$newline"/>
    <xsl:comment>End for slides</xsl:comment><xsl:value-of select="$newline"/>
</xsl:template>

<xsl:template name="addSliderInit">
   <xsl:variable name="empty" select="' '"/>
  <xsl:element name="script">
    $(function() {
      $('#slides').slidesjs({
...
<xsl:value-of select="$newline"/>
 <xsl:element name="script">
      <xsl:attribute name="type">text/javascript</xsl:attribute>
      <xsl:attribute name="src">jscripts/responsive.js</xsl:attribute>
	 <xsl:value-of select="$empty"/>
    </xsl:element>
<xsl:value-of select="$newline"/>
 <xsl:element name="script">
...
$(".section").attr('style','position: absolute; display: block !important;top:0;left 0;width:100%;z-index:10;backface-visibility:hidden;');

setTimeout( function(){ 
    $(".section").attr('style','position: absolute; display: none;top:0;left 0;width:100%;z-index:10;backface-visibility:hidden;');  
$(".section").first().attr('style','position: absolute; display: block;top:0;left 0;width:100%;z-index:99999;backface-visibility:hidden;');
$("#overlap").hide();
}, 5000 );
...
<xsl:value-of select="$newline"/>
<div id="overlap">
<img src="http://mdita.metropolitan.ac.rs/images/loader.gif"/>
</div>
</xsl:template>
...
<xsl:template name="addSliderNavigation">
<xsl:value-of select="$newline"/><xsl:comment>IRVAS slider navigation</xsl:comment>
<xsl:value-of select="$newline"/><a href="#" class="slidesjs-previous slidesjs-navigation"><i class="icon-chevron-left"><xsl:value-of select="$newline"/></i></a>
<xsl:value-of select="$newline"/><a href="#" class="slidesjs-next slidesjs-navigation"><i class="icon-chevron-right"><xsl:value-of select="$newline"/></i></a>
